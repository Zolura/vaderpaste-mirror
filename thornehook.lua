...
makefolder("thornehook/debug")
makefolder("thornehook/configs")
makefolder("thornehook/sounds")

for soundPath, soundUrl in {
	["thornehook/sounds/bubble.mp3"] = "https://github.com/Zolura/vaderpaste-mirror/raw/refs/heads/main/sfx/gmod_bubble.mp3",
	["thornehook/sounds/bubble2.mp3"] = "https://github.com/Zolura/vaderpaste-mirror/raw/refs/heads/main/sfx/gmod_bubble_2.mp3",
	["thornehook/sounds/trident.mp3"] = "https://github.com/Zolura/vaderpaste-mirror/raw/refs/heads/main/sfx/trident-new.mp3",
}
 do
...
pcall(function()
	local defaultConfig = readfile("thornehook/configs/main.value")
...
function get(url)
	return game:HttpGet(url) or request({
		...
		Method = "GET",
		Headers = {
			["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36 ThorneClient/1.0 Chocosploit/1.0"
		}
	}).Body
end
...
task.spawn(function()
	...
	icon.Transparency = 1
	icon.Image = "rbxassetid://94668572636885"
	icon.Parent = frame
	...
	if not isfile("thornehook/cache/assets.cache") then
		...
		for i, v in asset_list do
			...
			writefile(`thornehook/imgs/{i}`, src)

			fps.Text = "Caching Assets..".. (` ({n} / {l})`)
			task.wait(0.5)
		end
		writefile("thornehook/cache/assets.cache", "<translation=\"completed\">")
...
pcall(function()
	local defaultConfig = readfile(`thornehook/configs/{tostring(game.PlaceId)}.value`)
	if defaultConfig then
		configName = defaultConfig
	end
end)

for _,v in listfiles("thornehook") do
	if string.match(v, ".value") or string.match(v, ".vector") then
		local name = string.gsub(v, "thornehook/", "")
		local src = readfile(v)
		delfile(v)
		writefile(`thornehook/configs/{name}`, src)
	end
end
...
for _, file in listfiles("thornehook/sounds/") do
...
local themes = {
	...
	preset = {
		...
		button = rgb(180, 101, 245),
		button_alt = rgb(178, 152, 236)
	},
...
function library:window(properties)
	local cfg = {
		name = properties.name or properties.Name or "thorne.hook",
	...
	pcall(function()
		library.gui_scale = tonumber(readfile("thornehook/configs/default_scale.value"))
	end)
	...
	function library:set_visibility()
		...
		writefile("thornehook/configs/default_scale.value", tostring(library.gui_scale))
		library.guiVisibility:Fire(library.gui_visible)
			task.wait()
		guiDebounce = false
	end
...
function library:watermark(options)
	local cfg = {
		name = options.name or "thorne.hook";
	}
...
do
	...
	pcall(function()
		local savedFramePos = http_service:JSONDecode(readfile('thornehook/configs/watermark.vector'))
	...
	local renderFuncs = {
		...
		username = function()
			return thookAcc.username or "Thorne"
		end
	}
	...
	cons[#cons + 1] = services.runService.RenderStepped:Connect(function(dt)
		...
		if os.clock() - last > 0.85 then
			...
			writefile('thornehook/configs/watermark.vector', http_service:JSONEncode({
		...
		label.Text = `thorne<font color="{rgbstr(themes.preset.button)}">.hook</font>{activeText}`
		local interpValue = rgbstr( themes.preset.button:Lerp(rgb(), math.abs(0.1 - 0.05 * cos(sine / 30))) )
		corner_text.Text = `thorne<font color="{interpValue}">.hook</font>{activeText}`
	end)
end
...
local window = library:window({
	name = `thorne<font color="{rgbstr(themes.preset.button_alt)}">.hook</font> {build_str}`,
	size = dim2(0, 460, 0, 362)
})

checkfile('thornehook/cache/von.png', 'https://raw.githubusercontent.com/Zolura/vaderpaste-mirror/refs/heads/main/assets/image-removebg-preview.png')
...
if game.GameId == 113491250 then 
	...
	if not getgenv().replacementForModules and not getrenv().shared.require then
		task.spawn(messagebox, 'Failed to load PF Support.', 'thorne.hook', bit32.bxor(32,0,256))
...
overshoot_parent:add(fov_tab:slider({name = "overshoot fov", min = 0, max = 360, default = 100, flag = "pf_overshoot_fov"}))
	...
	bt_parent:add(bt_tab:colorpicker({name = "chams color", flag = "pf_backtrack_color", color = rgb(172, 120, 255), alpha = 0.5}))
	...
	modules.Tips = {"thorne.hook winning", "get good with thornehook", "thornehook p100 cheat"}
	...
	library.safe (function()
		do
			...
			local function deepServerHop()
				local CACHE_FILE = `thornehook/cache/server-history-{lp.UserId}.json`
	...
	local function onKillSuccess()
		local messages = {"tap in NOW with thornehook", "we love pasting", "top larpers use thornehook", "thornehook better", "what's hacking? i'm using a macro called thornehook", "need a gaming chair? get thorne.hook", "get owned by thornehook", "get good get thornehook", "thornehook > you", "need some fun? get thorne.hook", "powered by thornehook", "thornehook on top"}
			...
			if flags.chatlogs then
				if not Replication.getEntry(hitPlayer) then
					chatInterface.addMessage({
						isTeamChat = false;
						message = `<font color="{rgbstr(themes.preset.button)}">[thorne.hook]</font> Invalid or unavalible ReplicationObject for {hitPlayer.DisplayName}`;
						senderData = {
							senderType = "Console";
						};
					})
	...
	library.network:receive("bulletHitConfirm", function(hitPlayer, hitGroup, hitPos, damageAmount)
		...
		chatInterface.addMessage({
			isTeamChat = false;
			message = `<font color="{rgbstr(themes.preset.button)}">[thorne.hook]</font> hit {hitPlayer.DisplayName} for {floor(damageAmount)} in their {string.lower(hitGroup)}. [id={id}]`;
			senderData = {
				senderType = "Console";
			};
		})
		...
		if flags.world_nightmode == 'nightmode' then
			for _,v in workspace:GetDescendants() do
				if v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") then
					if not v:GetAttribute("thornehookBrightness") then
						v:SetAttribute("thornehookBrightness", v.Brightness)
					end
					v.Brightness = v:GetAttribute("thornehookBrightness") * .1
				end
			end
		elseif flags.world_nightmode == 'fullbright' then
			for _,v in workspace:GetDescendants() do
				if v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") then
					...
					v.Brightness = v:GetAttribute("thornehookBrightness") * .6
				end
			end
		else
			for _,v in workspace:GetDescendants() do
				if v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") then
					...
					v.Brightness = v:GetAttribute("thornehookBrightness")
				end
			end
		end
	end,
...
workspace.DescendantAdded:Connect(function(v)
	...
	v.Brightness = v:GetAttribute("thornehookBrightness") * .3
end)
...
library.theme:add(section:colorpicker({name = "ui color", flag = "ui_color", color = themes.preset.button, alpha = 0, callback = function()
	...
	task.delay(1/5, function()
		...
		window:set_title(`thorne<font color="{rgbstr(themes.preset.button_alt)}">.hook</font> {build_str} | {configName}`)
		for _, v in library.gui:GetDescendants() do
...
refresh_configs = function()
	...
	for idx, file in listfiles(library.directory .. "/configs") do
		...
		options[ #options + 1 ] = string.gsub(string.gsub(string.gsub(file :: string, ".cfg", ''), 'configs\\', ''), 'thornehook\\', '')
	end
...
section:button({ name = "set default (universal)", callback = function()
	...
	writefile("thornehook/configs/default.value", flags.config_list)
...
section:button({ name = "set default (game)", callback = function()
	...
	writefile(`thornehook/configs/{tostring(game.PlaceId)}.value`, flags.config_list)
...
refresh_luas = function()
	...
	for idx, file in listfiles(library.directory .. "/lua") do
		...
		options[ #options + 1 ] = string.gsub(string.gsub(string.gsub(file :: string, ".lua", ''), 'lua\\', ''), 'thornehook\\', '')
	end
...
local function start_lua(lua_name, should_load)
	...
	local loadfunc = loadstring(src, `[thornehook::{lua_name}]`)
...
createNotification({text = " Thornehook loaded"})
...
esp.config = {
	thornehook = `<font color="rgb(255, 164, 164)"><b>[S]</b></font>`,
...
function esp:create()
	return library:new("Frame",{
		...
		library:new("Frame", {
			...
			library:new("TextLabel", {
				Name = "username",
				Text = `{esp.config.thornehook} {esp.config.ax} username`,
				Size = UDim2.new(1,0,0,0),
				AutomaticSize = Enum.AutomaticSize.Y,
				FontFace = esp.config.font,
				TextStrokeTransparency = 0.5,
...
task.spawn(function()
	...
	task.spawn(function()
		...
		function addCharacterStuff(char, which)
			if not char:FindFirstChild("CharacterFieryHorns") and which == "fieryHorns" then
				...
				fiery.Handle.Fire.Color,fiery.Handle.Fire.SecondaryColor = rgb(201, 153, 255), rgb(188, 137, 255) 
	...
	function checkPlayer(player)
		if table.find(userIdList, player.UserId) then
			...
			img.Image = getcustomasset("thornehook/imgs/t.png")
			img.Parent = icon
...
task.spawn(function()
	...
	function createWidgetDrag(frame, frameName)
		local n = 'thornehook/configs/'..frameName..'.vector'
